...
public class ExampleTextInputPanel extends JPanel implements ActionListener {
	// These components are accessed from more than one method so they
	// must be member variables.
	private JLabel label;
	private JTextField text;
	private JButton button;
	/** Create panel containing the required components. */
	public ExampleTextInputPanel() {
		...
		add(text);
		add(button);
		// Call actionPerformed method of this object if text is entered
		// in field.
		text.addActionListener(this);
		button.addActionListener(this); // Likewise if button clicked.
	}
	/** This method is called by Swing whenever an action is detected in
	 * the JTextField or JButton. It is required by the ActionListener
	 * interface. */
